...
class VisionTransformer(nn.Module):
    def __init__(
            self, cfg ,input_shape=[64, 64], patch_size=16, in_chans=3, num_classes=200, num_features=768,
        ...
        super().__init__()
        self.act_layer = nn.GELU if cfg.model == "vit_gelu" else (nn.SiLU if cfg.model == "vit_silu" else nn.ReLU)
        #-----------------------------------------------#
        #   224, 224, 3 -> 196, 768
        #-----------------------------------------------#
...
def ViT(cfg, device):
    model = VisionTransformer(cfg,[cfg.inputSize, cfg.inputSize], in_chans= 3 if cfg.dataset != "mnist" else 1).to(device)
    if cfg.num_classes !=1000:
        model.head = nn.Linear(model.num_features, cfg.num_classes).to(device)
    return model